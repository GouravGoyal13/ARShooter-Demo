...
public class PlayerController : MonoBehaviour 
{
	...
	void Update ()
	{
		...
		{
			nextFire = Time.time + fireRate;
			Instantiate(shot, this.transform.position, shotSpawn.rotation);// as GameObject;
	...
	void FixedUpdate ()//physic update step function
	{
		...
		//
		#if UNITY_EDITOR
			GetComponent<Rigidbody>().velocity = movement * speed; //control speed of game object
		#endif
		//
		#if UNITY_WP_8_1
		...
		#if UNITY_ANDROID
		GetComponent<Rigidbody>().velocity = movementPhone * speed; //control speed of game object
		#endif
		//
